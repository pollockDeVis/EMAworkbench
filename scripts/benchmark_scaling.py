...
from models_to_benchmark import get_flu_model_instance
import timeit
...
if __name__ == '__main__':
    ...
    time_taken = timeit.repeat(
                lambda: run_model_with_evaluator(evaluator_class=MPIEvaluator, model_function=get_flu_model_instance, scenarios=n_scenarios),
                number=1,
                repeat=10)
    print(f"Time taken: {time_taken}")

    # Save results
    with open(f"benchmark_results_DelftBlue_scaling2_{node_multiplier}nodes_exclusive_flu.pickle", "wb") as handle:
